...
@Controller('jf/borrowers-sheets') // Legacy: Base route for borrowers (Sheets)
export class BorrowersControllerSheets {
  private readonly logger = new Logger(BorrowersControllerSheets.name);
  ...
  async getAllBorrowers(@Query('sslId') sslId?: string) {
    if (!sslId) {
      return {
        ...
        example: 'http://localhost:3000/jf/borrowers-sheets?sslId=YOUR_SSL_ID',
      };
    }
  ...
  @Get('/find') // GET /jf/borrowers-sheets/find?name=...
  ...
  @Get(':rowId') // GET /jf/borrowers-sheets/<rowId>
