...
import { ImpactSurveyDbService } from './impact-survey-db.service';

@Injectable()
export class ImpactSurveySyncService {
  private readonly logger = new Logger(ImpactSurveySyncService.name);
  ...
  async syncFromGoogleSheets(spreadsheetId: string) {
    try {
      ...
      // Get the sheet data
      const sheetData = await this.sheetsService.getImpactSurvey();
      ...
      for (const row of sheetData) {
        try {
          ...
          if (existingRecord) {
            ...
            await this.impactSurveyDbService.update(
              existingRecord.id,
              this.impactSurveyDbService.convertSheetToDb(row),
            );
  ...
  async syncToGoogleSheets(spreadsheetId: string) {
    try {
      ...
      const dbRecords = await this.impactSurveyDbService.findAll();
  ...
  async getSheetData(spreadsheetId: string) {
    try {
      return await this.sheetsService.getImpactSurvey();
    } catch (error) {
